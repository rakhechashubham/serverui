...
// (web) or SQLite (desktop) via crypto.Cipher / crypto.Box. They are never
// stored in the OS keychain and never returned to the frontend.
//
// Desktop: Tauri may load/store the encryption key via the OS keychain and
// inject it into the Go process environment. Go continues to consume the key
// only through env — it does not talk to the keychain itself.
//
